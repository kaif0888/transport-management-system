...
private RentalDetailsService rentalService;
 ...
 public RentalDetailsBean addRental(@RequestBody RentalDetailsBean bean) {
     ...
     public RentalDetailsBean updateRental(@PathVariable String id, @RequestBody RentalDetailsBean bean) {
     return rentalService.updateRental(id, bean);
     }

     @GetMapping("/track/{id}")
     public String trackRental(@PathVariable String id) {
     return rentalService.trackRentalStatus(id);
      }
     
     @GetMapping("/getRentalById/{id}")
     public ResponseEntity<RentalDetailsBean> getRentalById(@PathVariable("id") String rentalDetailsId) {
         try {
