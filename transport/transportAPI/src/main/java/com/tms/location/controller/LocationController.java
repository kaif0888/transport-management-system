...
public class LocationController {
    ...
    public ResponseEntity<LocationBean> getLocation(@PathVariable("id") String id) {
        try {
    ...
    public ResponseEntity<LocationBean> updateByLocationId(@PathVariable("id") String id, @RequestBody LocationBean locationBean) {
        try {
    ...
    @GetMapping("/getLocationsByPincode/{pincode}")
    public ResponseEntity<?> getLocationsByPincode(@PathVariable("pincode") String pincode) {
        List<PostOfficeResponse> postOffices = locationService.getPostOfficesByPincode(pincode);
        if (postOffices.isEmpty()) {
            return ResponseEntity.status(404).body("No post office found for pincode: " + pincode);
        }
        return ResponseEntity.ok(postOffices);
    }
    
    @GetMapping("/getLocationsAddressByPincode/{pincode}")
    public ResponseEntity<List<PostOfficeResponse>> getLocationsAddressByPincode(@PathVariable String pincode) {
        try {
            List<PostOfficeResponse> locations = locationService.getLocationsAddressByPincode(pincode);
            return ResponseEntity.ok(locations);
        } catch (Exception ex) {
            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
        }
